...
export function Sidebar({ isAdmin = false, currentUser }: { isAdmin?: boolean; currentUser?: CurrentUser | null }) {
  ...
  const navBottom = [
    ...
    { name: (t as any).billsNav ?? 'Bills', href: '/accounting/bills', icon: Wallet, adminOnly: true },
    { name: (t as any).reconciliationNav ?? 'Reconciliation', href: '/accounting/reconciliation', icon: Banknote, adminOnly: true },
