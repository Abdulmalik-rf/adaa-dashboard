...
export function BillEditor({ bill, ar }: { bill: any; ar: boolean }) {
  const editable = bill.status === 'draft'
  const [lines, setLines] = useState<Line[]>(() => {
    const raw = Array.isArray(bill.line_items) ? bill.line_items : []
    return raw.map((l: any) => ({
      description: l.description || '',
      qty: Number(l.qty ?? 1),
      unit_price: Number(l.unit_price ?? 0),
      vat_rate: Number(l.vat_rate ?? bill.vat_rate ?? 15),
  ...
  function addLine() {
    setLines((arr) => [
      ...arr,
      { description: '', qty: 1, unit_price: 0, vat_rate: Number(bill.vat_rate ?? 15), vat_amount: 0, line_total: 0 },
    ])
  }
  ...
  function save() {
    ...
    startTransition(async () => {
      const r = await updateDraftBill(bill.id, { line_items: recalc(lines), vat_rate: Number(bill.vat_rate ?? 15) })
